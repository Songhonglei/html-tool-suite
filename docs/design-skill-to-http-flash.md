...
## 2. 为什么是"直执行"（v2.0 的核心决策）

flash v1.x 曾走过"经 Agent/LLM 解析再执行"的老路，v2.0 彻底推翻，改为 subprocess 直执行。原因：

| 维度 | 经 Agent/LLM 执行 | subprocess 直执行（v2.0）|
|---|---|---|
...
结论：一个"输入结构化参数、输出结构化结果"的 skill，根本不需要 LLM 参与执行。
