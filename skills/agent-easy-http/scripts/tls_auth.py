...
注：v3.0 后 agent-easy-http 走 OpenClaw 原生 /hooks/agent，不再用 callback HMAC，
    但下面的 HMAC 函数（compute_callback_signature / verify_callback_signature）
    保留作为**模板**，给 skill-to-http 等需要自有 callback 链路的 skill 复用。
"""
...
class TLSAuthConfig:
    ...
    注：callback_* 字段是为有自有 callback 链路的 skill 准备的（如 skill-to-http
    可能需要在 sub-agent 回调时做 HMAC 校验）。agent-easy-http v3.0 走 OpenClaw
    原生 /hooks/agent，不再使用 callback，相关字段保持默认即可。
    """
    # TLS（默认关闭 - 证书生成/客户端导入有门槛，按需开启）
    ...
    # Callback HMAC（可选 - 仅当 skill 有自有 callback 链路时使用）
    callback_auth_enabled: bool = False  # v3.0 默认关闭
    callback_secret: str = ""
    ...
    def from_dict(cls, d: dict) -> "TLSAuthConfig":
        ...
        return cls(
            ...
            callback_auth_enabled=d.get("callback_auth_enabled", False),  # v3.0 默认 false
...
def save_api_key(skill_name: str, key: str) -> Path:
    """保存 API Key 到 ~/.http/secrets/api-keys/<skill>.key（0600）。"""
    ensure_dirs()
...
def save_hmac_secret(skill_name: str, secret: str) -> Path:
    """保存 HMAC secret 到 ~/.http/secrets/<skill>.hmac（0600）。"""
    ensure_dirs()
...
def validate_config(cfg: TLSAuthConfig, skill_name: str) -> list[str]:
    ...
    if not cfg.api_key:
        errors.append(
            ...
            f"或自行设置后写入 ~/.http/secrets/api-keys/{skill_name}.key"
        )
